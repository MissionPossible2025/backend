...
const cleanPhotoArray = (photosInput) => {
  ...
  for (const raw of arr) {
    ...
    if (!trimmed) continue;
    // Skip base64 / data URLs (used only as local previews, should never be stored)
...
export const createProduct = async (req, res) => {
  try {
    ...
    // Safety check: if files were sent but no ImageKit URLs were produced, abort instead of saving a product without images
    if (req.files && Object.keys(req.files).length > 0 && !photoUrl && photoUrls.length === 0) {
      console.error('[createProduct] Files were received but no ImageKit URLs were created. Aborting product save.');
      return res.status(500).json({
        error: 'Images were received but failed to upload to ImageKit. Product was not saved. Please try again.'
      });
    }
