...
export default function EditItem() {
  ...
  // Helper to normalize, de-duplicate, and sanitize photo URL arrays (frontend)
  const cleanPhotoArray = (photosInput) => {
    ...
    for (const raw of arr) {
      ...
      if (!trimmed) continue;
      // Skip base64 / data URLs when cleaning URLs from API or when
      // building existingPhotos for the backend. These are local previews.
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([productData]) => {
        ...
        // Set photos if they exist (sanitize and de-duplicate)
        if (productData.photos && productData.photos.length > 0) {
          setPhotos(cleanPhotoArray(productData.photos));
        } else if (productData.photo) {
          setPhotos(cleanPhotoArray([productData.photo]));
        }
        setLoading(false);
      })
      .catch(err => {
  ...
  const handleSave = async () => {
    ...
    const existingPhotoUrls = cleanPhotoArray(photos.filter(photo => typeof photo === 'string'));
    ...
    try {
      ...
      if (response.data) {
        ...
        // Replace local state with cleaned photos from API response
        setItem(updatedProduct);
        if (updatedProduct.photos && updatedProduct.photos.length > 0) {
          setPhotos(cleanPhotoArray(updatedProduct.photos));
        } else if (updatedProduct.photo) {
          setPhotos(cleanPhotoArray([updatedProduct.photo]));
        } else {
          setPhotos([]);
        }
        setPhotoFiles([]);
        alert("Product updated successfully!");
        navigate("/", { state: { activeTab: "products" } });
      }
    } catch (err) {
  ...
  return (
    ...
    <div style={{ 
      ...
      <div style={{
        ...
        <h2 style={{ marginBottom: "2rem", fontSize: "clamp(1.5rem, 5vw, 2rem)", textAlign: "center" }}>Edit Product</h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem", width: "100%", boxSizing: "border-box" }}>
          ...
          {!item.hasVariations && (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "1rem", marginTop: "1rem", width: "100%", boxSizing: "border-box" }}>
                <div>
                  <label style={{ display: "block", marginBottom: "0.5rem", fontWeight: "500" }}>Tax Percentage (%):</label>
                  <input 
                    type="number" 
