...
// Order IDs to hide from display (data stays in backend)
const ORDERS_TO_HIDE = ['698c2876b4986efe0a0aa38c'];
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        // Filter orders to show only items from this seller, and hide specific orders
        const ordersWithFilteredItems = (response.data.orders || [])
          .filter(order => !ORDERS_TO_HIDE.includes(order._id))
          .map(order => ({
            ...order,
  ...
  return (
    <div style={{ 
      ...
      <div style={{
        ...
        <h2 style={{ marginBottom: "clamp(1rem, 3vw, 2rem)", fontSize: "clamp(1.5rem, 5vw, 2rem)" }}>Order Management</h2>
                ...
                {orders.map(order => (
                  <div key={order._id} style={{
                    ...
                    <div style={{ display: 'grid', gap: '0.5rem' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '0.5rem', width: '100%', boxSizing: 'border-box' }}>
                          <button
