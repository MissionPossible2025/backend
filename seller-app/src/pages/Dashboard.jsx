...
export default function Dashboard({ user }) {
  ...
  // Ensure we can land directly on the "View Products" tab when requested
  useEffect(() => {
    if (location.state?.activeTab === 'products') {
      setActiveTab('products');
    }
  }, [location.state]);

  // Restore products list scroll position after returning from edit screen (once list has rendered)
  useEffect(() => {
    if (activeTab !== 'products' || loading) return;
    const saved = sessionStorage.getItem('seller_dashboard_products_scroll');
    if (saved === null) return;
    const value = parseInt(saved, 10);
    sessionStorage.removeItem('seller_dashboard_products_scroll');
  ...
  return (
    <div style={{ 
      ...
      <div style={{
        ...
        <h1 style={{ marginBottom: "1rem", fontSize: "clamp(1.5rem, 5vw, 2.5rem)", wordBreak: "break-word" }}>Welcome, {user.name}</h1>
        <h2 style={{ marginBottom: "2rem", fontSize: "clamp(1.1rem, 4vw, 1.5rem)", color: "#888" }}>Seller Dashboard</h2>
        ...
        {activeTab === 'products' && (
          <div
            ...
            style={{ textAlign: "left", maxHeight: "70vh", overflowY: "auto", width: "100%", boxSizing: "border-box" }}
          >
                          ...
                          <button
                            onClick={() => {
                              if (productsScrollRef.current) {
                                const current = productsScrollRef.current.scrollTop || 0;
                                sessionStorage.setItem('seller_dashboard_products_scroll', String(current));
                              }
