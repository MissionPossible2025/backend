...
// Helper to normalize, de-duplicate, and sanitize photo URL arrays from a product.
// - Handles null/undefined product safely
// - Trims URLs
// - Removes duplicates
// - Ignores empty strings
// - Excludes base64/data URLs (e.g. local preview images: data:image/...)
const getUniqueValidPhotos = (product) => {
  if (!product) return []
  ...
  for (const url of raw) {
    ...
    if (!trimmed) continue
    // Skip base64 / data URLs used for local previews
...
export default function ProductsList({ searchTerm: externalSearchTerm = '' }) {
  ...
  const handleTouchEnd = () => {
    ...
    if (isLeftSwipe && selectedProduct) {
      const uniquePhotos = getUniqueValidPhotos(selectedProduct)
      setCurrentImageIndex(prev => prev < uniquePhotos.length - 1 ? prev + 1 : 0)
    }
    if (isRightSwipe && selectedProduct) {
      const uniquePhotos = getUniqueValidPhotos(selectedProduct)
